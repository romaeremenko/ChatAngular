...
export class AuthorizationUserService {
  ...
  login({loginField, passwordField}): void {
    this.authService.isExist(loginField, passwordField).subscribe((resp: UserResponce) => {
      this.redirectToChatService.redirectTo(resp, loginField, passwordField, ChatMessagesService.room.name.split(' ').join(''));
      this.isAuth = true;
    }, resp => {
  ...
  registration(loginField, passwordField): void {
    this.authService.registration(loginField, passwordField).subscribe((resp: UserResponce) => {
      this.redirectToChatService.redirectTo(resp, loginField, passwordField, 'Main');
      this.isAuth = true;
    }, resp => {
      this.responce = resp.error.message;
    });
